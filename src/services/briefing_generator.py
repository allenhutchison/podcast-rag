...
# Prompt for multiple episodes (cross-episode synthesis)
_MULTI_EPISODE_PROMPT = """\
...
# Prompt for a single episode (depth instead of synthesis)
_SINGLE_EPISODE_PROMPT = """\
You are a senior podcast analyst writing the lead section of a daily newsletter \
...
def generate_digest_briefing(episodes: list, config: Config) -> dict | None:
    ...
    try:
        ...
        # Choose prompt based on episode count
        if len(episodes) == 1:
            prompt = _SINGLE_EPISODE_PROMPT.format(episodes_block=episodes_block)
        else:
            prompt = _MULTI_EPISODE_PROMPT.format(episodes_block=episodes_block)
        ...
        if store_name:
            ...
            grounded_text = grounded_response.text if grounded_response.text else ""
