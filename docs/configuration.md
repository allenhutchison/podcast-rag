...
## Transcription Backend (Scribe)

Podcast RAG uses the shared Scribe transcription service for all transcription.

| Variable | Default | Description |
|----------|---------|-------------|
| `SCRIBE_BASE_URL` | `https://scribe.vycari.ai` | HTTPS base URL of the Scribe API |
| `SCRIBE_API_TOKEN` | — | Bearer token for the Scribe API; **required** |
